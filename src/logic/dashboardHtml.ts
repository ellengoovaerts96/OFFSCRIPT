...
<head>
  ...
  <link rel="manifest" href="/admin-assets/manifest.webmanifest?v=4">
...
<body>
  ...
  <main>
    ...
    <section class="workspace" aria-label="TUUTI workspace">
      <article class="card primary"><h2>Field Research</h2><p class="purpose">Review Structured Import proposals, edit details and approve them for Places.</p><div class="actions"><a class="action" href="/admin/field-research">Review Structured Import<span aria-hidden="true">→</span></a>${externalAction(config.fieldResearchFormUrl, "Add field research")}${externalAction(config.fieldResearchInboxUrl, "Open research inbox")}</div></article>
      ...
      <article class="card"><h2>Events</h2><p class="purpose">Import event screenshots, verify their details and save event drafts.</p><div class="actions"><a class="action" href="/admin/events">Manage events<span aria-hidden="true">→</span></a><a class="action" href="/admin/events/import">+ Import from screenshot</a></div></article>
...
export function renderStagingTest(csrf = ""): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#17201d"><link rel="manifest" href="/admin-assets/manifest.webmanifest?v=4"><link rel="apple-touch-icon" href="/apple-touch-icon.png?v=4"><link rel="icon" type="image/png" href="/admin-assets/tuuti_dashboard_flavicon.png"><title>Test TUUTI · Staging</title><style>
  ...
  *{box-sizing:border-box} body{margin:0;min-width:320px} header{display:flex;align-items:center;justify-content:space-between;padding:22px max(18px,calc((100% - 780px)/2));border-bottom:1px solid rgba(242,235,217,.16)} header a,header button{color:var(--cream);text-decoration:none}.mobile-back{display:none;border:0;background:transparent;font:inherit;font-size:22px;cursor:pointer}.badge{padding:6px 9px;border:1px solid #9fa581;border-radius:999px;font-size:11px;font-weight:750;letter-spacing:.1em} main{width:min(780px,calc(100% - 28px));margin:42px auto} h1{margin:0;font:500 clamp(36px,7vw,56px)/1 Canela,Georgia,serif}.intro{color:#bbb5a5;line-height:1.55}.chat{margin:28px 0;padding:20px;border-radius:14px;background:var(--paper);color:var(--deep)}#messages{display:grid;gap:10px;min-height:180px;margin-bottom:18px}.message{max-width:85%;padding:11px 13px;border-radius:10px;line-height:1.45;white-space:pre-wrap}.tuuti{background:#eee5d0}.user{justify-self:end;color:white;background:var(--rouge)}form{display:flex;gap:9px}input{min-width:0;flex:1;padding:12px 13px;border:1px solid #c9c0ac;border-radius:7px;font:inherit}button{padding:11px 18px;border:0;border-radius:7px;color:white;background:var(--deep);font:inherit;font-weight:700;cursor:pointer}button:disabled{opacity:.55}@media(max-width:520px){.desktop-back{display:none}.mobile-back{display:inline-flex;align-items:center}form{flex-direction:column}button{min-height:44px}}
  </style></head><body><header><button class="mobile-back" type="button" aria-label="Go back" onclick="if(history.length>1){history.back()}else{location.href='/admin'}">←</button><a class="desktop-back" href="/admin">← TUUTI Operations</a><span class="badge">STAGING</span></header><main><h1>Test TUUTI</h1><p class="intro">Send a message through the staging chatbot. Use fictional test details only. This tool is available on staging and uses a separate test identity.</p><section class="chat"><div id="messages"><div class="message tuuti">What would you like to discover in Dakar?</div></div><form id="form"><input id="message" autocomplete="off" placeholder="Type a test message…" required><button type="submit">Send</button></form></section></main><script>
