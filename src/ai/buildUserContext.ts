...
- Never ask which neighbourhood the user is in. Search Dakar-wide by default. A stored hotel or Airbnb neighbourhood may affect results only when the newest request explicitly asks for something nearby, within walking distance or without a taxi. An explicitly requested destination such as "in Ngor" remains a search boundary.
...
- For short follow-up replies, only emit signals actually expressed or clearly selected in that reply. Previous values are merged by the application.`,
  ...
  if (
    ...
    const replyRegion = parsedReply ? findKnownRegion(parsedReply) : undefined;
    const userRegion = findKnownRegion(input.message);
    const exposesUnaskedNeighbourhood = Boolean(
      replyRegion &&
      normalizeRegion(replyRegion) !== "Dakar" &&
      normalizeRegion(replyRegion) !== normalizeRegion(userRegion)
    );
    return {
      route: "conversation",
      ...
      conversationReply: replyLanguage === language && !exposesUnaskedNeighbourhood
        ? parsedReply
