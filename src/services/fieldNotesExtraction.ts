// Uses the same extraction schema and editorial rules as scripts/processFieldNotes.ts.
// Sheet writes and dashboard approval are deliberately separate from extraction.
import { zodTextFormat } from "openai/helpers/zod";
import { z } from "zod";
import { getOpenAIClient, openaiModel } from "../integrations/openai.js";
...
import {
  ...
  hasAssessmentAnswer
} from "../logic/fieldNotesAssessment.js";
...
export function explicitAssessment(source: Record<string, unknown>): Partial<StructuredNote> {
...
export function structuredRow(sourceId: string, timestamp: string, visitDate: string, researcher: string, note: StructuredNote): Cell[] {
...
export async function structureDraft(input: Record<string, unknown>): Promise<StructuredNote> {
  const response = await getOpenAIClient({ timeoutMs: 25000, maxRetries: 0 }).responses.parse({
    model: openaiModel,
